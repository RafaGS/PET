...
uint8_t VIA::read_portb() {
	// Output bits (DDRB=1): read from output latch _portb
	// Input bits  (DDRB=0): read from external pin state _portb_in (or handler)
	uint8_t ib = _portb_input_handler ? _portb_input_handler() : _portb_in;
	return (_portb & _ddrb) | (ib & ~_ddrb);
}
...
void VIA::write_portb_in_bit(uint8_t bit, bool state) {
	// Writes to the INPUT pin register, not the output latch
	if (state)
		_portb_in |= bit;
	else
		_portb_in &= ~bit;
