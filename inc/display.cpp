...
// Las macros A,C,D,E,O de CPU.h colisionan con métodos de bitluni Graphics.h
// VGA_RESOLUTION NO se toca — es necesario para VGAMode::VGA_RESOLUTION
#pragma push_macro("A")
#pragma push_macro("C")
#pragma push_macro("D")
#pragma push_macro("E")
...
#if VGA_BIT_DEPTH == 1
    // GraphicsW1: ColorW1X7 con colormask=0x01
    // clear() usa (color & 0x1) — el único bit que importa es el bit 0
    // Pixel encendido = 1, apagado = 0
    return (c == 0) ? 0 : 1;
#else
    uint8_t r = (c >> 11) & 0x1F;
    uint8_t g = (c >> 5)  & 0x3F;
...
void Display::begin(uint16_t bg, uint16_t fg, orientation_t orient) {
	...
	static bool init;
	if (!init) {
		Serial.println("ABOUT TO INIT VGA");
		Serial.flush();
		delay(1000);
#if VGA_BIT_DEPTH == 6
		...
		vga.show();  // inicializa frontBuffer/backBuffer antes de cualquier escritura
		init = true;
	}
